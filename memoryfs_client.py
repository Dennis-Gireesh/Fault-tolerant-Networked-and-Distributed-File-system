...
CHECKSUM_BLOCK_COUNT = TOTAL_NUM_BLOCKS/(BLOCK_SIZE/16) 
# 16 bytes = 128bits/8bits 
#128bits = each checksum hash size
...
class DiskBlocks():
  def __init__(self, args):
    ...
    if args.Number_of_servers:
      self.n_servers = args.Number_of_servers
    else:
      print('Must specify Number of Servers')
      quit() 
    #self.blocks_per_server = int(TOTAL_NUM_BLOCKS//(self.n_servers-1))
    self.activeports = {}
    ...
    if args.port0 or args.port1 or args.port2 or args.port3 or args.port4 or args.port5 or args.port6 or args.port7:
      if args.port0:  
        PORT0 = args.port0
        server_url0 = 'http://' + SERVER_ADDRESS + ':' + str(PORT0)
        self.block_server0 = xmlrpc.client.ServerProxy(server_url0, use_builtin_types=True)
        self.activeports[0] = PORT0
      if args.port1:
        PORT1 = args.port1
        server_url1 = 'http://' + SERVER_ADDRESS + ':' + str(PORT1)
        self.block_server1 = xmlrpc.client.ServerProxy(server_url1, use_builtin_types=True)
        self.activeports[1] = PORT1
      if args.port2:
        PORT2 = args.port2
        server_url2 = 'http://' + SERVER_ADDRESS + ':' + str(PORT2)
        self.block_server2 = xmlrpc.client.ServerProxy(server_url2, use_builtin_types=True)
        self.activeports[2] = PORT2
      if args.port3:
        PORT3 = args.port3
        server_url3 = 'http://' + SERVER_ADDRESS + ':' + str(PORT3)
        self.block_server3 = xmlrpc.client.ServerProxy(server_url3, use_builtin_types=True)
        self.activeports[3] = PORT3 
      if args.port4:
        PORT4 = args.port4
        server_url4 = 'http://' + SERVER_ADDRESS + ':' + str(PORT4)
        self.block_server4 = xmlrpc.client.ServerProxy(server_url4, use_builtin_types=True)
        self.activeports[4]= PORT4
      if args.port5:
        PORT5 = args.port5
        server_url5 = 'http://' + SERVER_ADDRESS + ':' + str(PORT5)
        self.block_server5 = xmlrpc.client.ServerProxy(server_url5, use_builtin_types=True)
        self.activeports[5] = PORT5
      if args.port6:
        PORT6 = args.port6
        server_url6 = 'http://' + SERVER_ADDRESS + ':' + str(PORT6)
        self.block_server6 = xmlrpc.client.ServerProxy(server_url6, use_builtin_types=True)
        self.activeports[6] = PORT6
      if args.port7:
        PORT7 = args.port7
        server_url7 = 'http://' + SERVER_ADDRESS + ':' + str(PORT7)
        self.block_server7 = xmlrpc.client.ServerProxy(server_url7, use_builtin_types=True)
        self.activeports[7] = PORT7
    else:
      print('Must specify port number')
      quit()
    if self.n_servers > len(self.activeports):
      print('Specify all the port properly')
      quit()
...
# RAID 5 mapping function
  def MappingFunc(self,block_number):
        block_server_id = int(block_number % (self.n_servers-1))
        ph_block_number = int(block_number //(self.n_servers -1))
        parity_server_id = (self.n_servers -1) - (ph_block_number % self.n_servers)
        if block_server_id >= parity_server_id:
              block_server_id +=1
        return ph_block_number,block_server_id,parity_server_id
...
## Blocks are padded with zeroes up to BLOCK_SIZE
  ...
  def Put(self, block_number, block_data):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS): 
      ...
      ph_block_number,block_server_id,parity_server_id = self.MappingFunc(block_number)
      try:
        if block_server_id <= self.n_servers:
            if block_server_id == 0:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata)
                  ret = self.block_server0.Put(ph_block_number,putdata)
            elif block_server_id == 1:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server1.Put(ph_block_number,putdata)
            elif block_server_id == 2:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server2.Put(ph_block_number,putdata)
            elif block_server_id == 3:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server3.Put(ph_block_number,putdata)
            elif block_server_id == 4:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server4.Put(ph_block_number,putdata)
            elif block_server_id == 5:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server5.Put(ph_block_number,putdata)
            elif block_server_id == 6:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server6.Put(ph_block_number,putdata)
            elif block_server_id == 7:
                  self.UpdateParity(ph_block_number,block_server_id,parity_server_id,putdata )
                  ret = self.block_server7.Put(ph_block_number,putdata)
            if ret == -1:
        ...
        logging.debug("Connection error while updating the data ---  updated the respective parity")
        pass
...
#updation of parity for every put
  def UpdateParity(self,ph_block_number,block_server_id,parity_server_id,new_data ):
        ...
        try:
            if block_server_id == 0:
                  old_data = self.block_server0.Get(ph_block_number)
            elif block_server_id == 1:
                  old_data = self.block_server1.Get(ph_block_number)
            elif block_server_id == 2:
                  old_data = self.block_server2.Get(ph_block_number)
            elif block_server_id == 3:
                  old_data = self.block_server3.Get(ph_block_number)
            elif block_server_id == 4:
                  old_data = self.block_server4.Get(ph_block_number)
            elif block_server_id == 5:
                  old_data = self.block_server5.Get(ph_block_number)
            elif block_server_id == 6:
                  old_data = self.block_server6.Get(ph_block_number)
            elif block_server_id == 7:
                  old_data = self.block_server7.Get(ph_block_number)
        ...
        try:    
            if parity_server_id == 0:
                  old_parity = self.block_server0.Get(ph_block_number)
            elif parity_server_id == 1:
                  old_parity = self.block_server1.Get(ph_block_number)
            elif parity_server_id == 2:
                  old_parity = self.block_server2.Get(ph_block_number)
            elif parity_server_id == 3:
                  old_parity = self.block_server3.Get(ph_block_number)
            elif parity_server_id == 4:
                  old_parity = self.block_server4.Get(ph_block_number)
            elif parity_server_id == 5:
                  old_parity = self.block_server5.Get(ph_block_number)
            elif parity_server_id == 6:
                  old_parity = self.block_server6.Get(ph_block_number)
            elif parity_server_id == 7:
                  old_parity = self.block_server7.Get(ph_block_number)
        ...
        intermediate_result =  bytes([aa ^ bb for aa, bb in zip(bytes(new_data),bytes( old_data))])
        new_parity = bytearray(bytes([aa ^ bb for aa, bb in zip(bytes(intermediate_result), bytes(old_parity))]))
        
        try:      
            if parity_server_id == 0:
                  ret = self.block_server0.Put(ph_block_number,new_parity)
            elif parity_server_id == 1:
                  ret = self.block_server1.Put(ph_block_number,new_parity)
            elif parity_server_id == 2:
                  ret = self.block_server2.Put(ph_block_number,new_parity)
            elif parity_server_id == 3:
                  ret = self.block_server3.Put(ph_block_number,new_parity)
            elif parity_server_id == 4:
                  ret = self.block_server4.Put(ph_block_number,new_parity)
            elif parity_server_id == 5:
                  ret = self.block_server5.Put(ph_block_number,new_parity)
            elif parity_server_id == 6:
                  ret = self.block_server6.Put(ph_block_number,new_parity)
            elif parity_server_id == 7:
                  ret = self.block_server7.Put(ph_block_number,new_parity)
            if ret == -1:
              logging.error('Put: Server returns error')
              quit()
        except ConnectionRefusedError:
            logging.debug("Connection error while updating the parity")
            pass
...
#repair method
  def repair(self,server_id):
      ...
      try:
        for block_num in range(TOTAL_NUM_BLOCKS//(self.n_servers-1)):
            corrected_data = bytearray(BLOCK_SIZE)
            for i in range(self.n_servers):
              if server_id != i: 
                if i == 0:
                      data = self.block_server0.Get(block_num)
                elif i == 1:
                      data = self.block_server1.Get(block_num)
                elif i == 2:
                      data = self.block_server2.Get(block_num)
                elif i == 3:
                      data = self.block_server3.Get(block_num)
                elif i == 4:
                      data = self.block_server4.Get(block_num)
                elif i == 5:
                      data = self.block_server5.Get(block_num)
                elif i == 6:
                      data = self.block_server6.Get(block_num)
                elif i == 7:
                      data = self.block_server7.Get(block_num)
                if data == -1:
                  logging.debug("Correcting block: Unknown error")
                  quit() 
                corrected_data =  bytes([aa ^ bb for aa, bb in zip(bytes(corrected_data), bytes(data))])
                
            if server_id == 0:
                  ret = self.block_server0.Put(block_num,bytearray(corrected_data))
            elif server_id == 1:
                  ret = self.block_server1.Put(block_num,bytearray(corrected_data))
            elif server_id == 2:
                  ret = self.block_server2.Put(block_num,bytearray(corrected_data))
            elif server_id == 3:
                  ret = self.block_server3.Put(block_num,bytearray(corrected_data))
            elif server_id == 4:
                  ret = self.block_server4.Put(block_num,bytearray(corrected_data))
            elif server_id == 5:
                  ret = self.block_server5.Put(block_num,bytearray(corrected_data))
            elif server_id == 6:
                  ret = self.block_server6.Put(block_num,bytearray(corrected_data))
            elif server_id == 7:
                  ret = self.block_server7.Put(block_num,bytearray(corrected_data))
            if ret != None:
              if ret == -1:
                logging.error('Put: Server returns error')
                quit()
        ...
        logging.error("Encountered two faults...")
        quit()
  
# Method to retrive data when server get failed
  def CreateFailedDataOnGet(self,ph_block_number,server_id):
      data_new = bytearray(BLOCK_SIZE) #initially zero bytearray so xor wont effect
      try:
          for i in range(self.n_servers):
                if i != server_id:
                  if i == 0:
                        data = self.block_server0.Get(ph_block_number)
                  elif i == 1:
                        data = self.block_server1.Get(ph_block_number)
                  elif i == 2:
                        data = self.block_server2.Get(ph_block_number)
                  elif i == 3:
                        data = self.block_server3.Get(ph_block_number)
                  elif i == 4:
                        data = self.block_server4.Get(ph_block_number)
                  elif i == 5:
                        data = self.block_server5.Get(ph_block_number)
                  elif i == 6:
                        data = self.block_server6.Get(ph_block_number)
                  elif i == 7:
                        data = self.block_server7.Get(ph_block_number)
                  if data == -1:
                       logging.error("Encounterd two fauls in system")
                       quit()
                  data_new =  bytes([aa ^ bb for aa, bb in zip(bytes(data_new), bytes(data))])
          return bytearray(data_new)
      except ConnectionRefusedError:
        logging.error("Encounterd two fauls in system")
        quit()                
...
## Equivalent to the textbook's BLOCK_NUMBER_TO_BLOCK(b)
  ...
  def ServerGet(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      #block_server_id = int(block_number // self.blocks_per_server)
      #server_block_number = int(block_number % self.blocks_per_server)
      ph_block_number,block_server_id,parity_server_id = self.MappingFunc(block_number)
      try:
          if block_server_id <= self.n_servers:
                if block_server_id == 0:
...
## RSM: read and set memory equivalent
  def CorrectDataBlock(self,block_server_id,ph_block_number):
      corrected_data = bytearray(BLOCK_SIZE)
      for i in range(self.n_servers):
        if block_server_id != i:  
          if i == 0:
                data = self.block_server0.Get(ph_block_number)
          elif i == 1:
                data = self.block_server1.Get(ph_block_number)
  ...
  def RSM(self, block_number):
    ...
    if block_number in range(0,TOTAL_NUM_BLOCKS):
      ...
      if block_server_id <= self.n_servers:
            if block_server_id == 0:
                  data = self.block_server0.RSM(ph_block_number)
            elif block_server_id == 1:
                  data = self.block_server1.RSM(ph_block_number)
            elif block_server_id == 2:
                  data = self.block_server2.RSM(ph_block_number)
            elif block_server_id == 3:
                  data = self.block_server3.RSM(ph_block_number)
            elif block_server_id == 4:
                  data = self.block_server4.RSM(ph_block_number)
            elif block_server_id == 5:
                  data = self.block_server5.RSM(ph_block_number)
            elif block_server_id == 6:
                  data = self.block_server6.RSM(ph_block_number)
            elif block_server_id == 7:
                  data = self.block_server7.RSM(ph_block_number)
      return bytearray(data)
